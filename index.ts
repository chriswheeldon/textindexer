...
class indexer {
  ...
  public index(filename: string, keyfunc: (line: string) => string): Promise<index> {
  ...
  private process(line: string) {
    ...
    if (line != this._prefix && this._stack.length < 10) {
      this.enter(line);
    }
    this._lineno += 1;
  }
...
export class txtindx {
  ...
  public index(): Promise<index> {
    return new indexer().index(this._filename, this._keyfunc);
  }
